...
const networkRequestViaProxy = (destinationUrlString, proxyUrlString, httpMethod, options, agentOptions, timeout) => {
    ...
    if (agentOptions && Object.keys(agentOptions).length) {
        tunnelRequestOptions.agent = new http__default["default"].Agent(agentOptions);
    }
    ...
    return new Promise((resolve, reject) => {
        const request = http__default["default"].request(tunnelRequestOptions);
        if (timeout) {
            request.on("timeout", () => {
                request.destroy();
...
const networkRequestViaHttps = (urlString, httpMethod, options, agentOptions, timeout) => {
    ...
    if (agentOptions && Object.keys(agentOptions).length) {
        customOptions.agent = new https__default["default"].Agent(agentOptions);
    }
    if (isPostRequest) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (customOptions.protocol === "http:") {
            request = http__default["default"].request(customOptions);
        }
        else {
            request = https__default["default"].request(customOptions);
        }
        if (isPostRequest) {
            request.write(body);
        }
